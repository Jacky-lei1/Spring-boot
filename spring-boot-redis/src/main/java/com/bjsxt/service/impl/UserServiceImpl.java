...
public class UserServiceImpl implements UserService {
    ...
    @Override
    @Cacheable(value = "users",keyGenerator = "RedisCacheConfig")
    public List<Users> findUserAll() {
        return usersRepository.findAll();
    }

    @Override
    @Cacheable(value = "users") //表示对当前查询的对象做缓存处理,value值表示自定义的缓存配置的策略名称
    ...
    @CacheEvict(value = "users",allEntries = true) //每次插入后为了能够重新查询(数据能够正确显示)，应该先清除上一次缓存再重新进行查询
